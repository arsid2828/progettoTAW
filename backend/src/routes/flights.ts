  ...
  /*  if (!from || !to || !date) {
   ...
   // console.log(`🔎 Ricerca voli da ${originAirport.name} a ${destAirport.name} il ${startOfDay.toISOString().split('T')[0]}`);
    ...
    const directFlights = await Flight.find({
      ...
      date_departure: { $gte: startOfDay, $lte: endOfDay }*/ // TODO: Uncomment and use these filters
    })
      .populate('airline')
    ...
    const results = directFlights.map(f => ({
      type: 'DIRECT',
      total_price: 100, // TODO: Qui dovresti prendere il prezzo dal SeatType (minimo)
